...
export default async function Header() {
    ...
    return <div>
        ...
        <nav className="h-12 px-[5%] w-full bg-zinc-100 flex gap-6 uppercase">
            ...
            {collections.map((collection: CollectionData, idx: number) =>
                <div key={idx} className="group">
                    <Link href={`/collection/${collection.slug}`} className="nav-link">{collection.label}</Link>
                    {collection.subcollections.length > 0 &&
                        <div className="absolute group-hover:flex flex-col hidden p-2 bg-white border-b-2 border-zinc-700">
                            {collection.subcollections.map((sub, idx) =>
                                <Link key={idx} href={`/collection/${collection.slug}/${sub.slug}`} className="nav-link border-none text-zinc-500 hover:text-zinc-800">
                                    {sub.label}
                                </Link>
                            )}
                        </div>}
